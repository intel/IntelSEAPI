...
import sea
from sea_runtool import Collector, get_decoders, is_domain_enabled, message, get_original_env
...
def bracket_hook_kmd(module='', function=''):
    res = []
    for item in bracket_hook(module, function):
        res.append(item.replace('pid$target:', 'fbt:').replace('/*{UMD_STACKS}*/', '/*{KMD_STACKS}*/').replace('/*{CONDITIONS}*/', ''))
    return res
...
HOTSPOTS = bracket_hook()  # all modules and functions

STATS = r"""
pid$target:::entry
{
    self->hotspots[probemod, probefunc] = machtimestamp;
}

pid$target:::return
/self->hotspots[probemod, probefunc]/
{
    @hotspots[probemod, probefunc] = avg(machtimestamp - self->hotspots[probemod, probefunc]);
    self->hotspots[probemod, probefunc] = 0;
}
...
GREEDY_KMD = GREEDY_UMD.replace('pid$target:', 'fbt:')
...
class DTraceCollector(Collector):
    ...
    def __init__(self, args):
        ...
        self.script = None
        if args.follow:
            self.fifo_reader = FifoReader(self, os.path.join(self.args.output, 'follow_child.fifo'))
        self.prepare()
        self.times = []
    ...
    def gen_gpu_hooks(self, text):  # TODO: check /System/Library/Extensions/AppleIntelSKLGraphics.kext/Contents/Info.plist
        ...
        for line in text.split('\n'):
            ...
            if 'dtHook' in name and parts[-1] == 'entry' and is_domain_enabled('dtHook'):
                ...
                stack = '' if not self.args.debug else (STACKS['UMD'] + STACKS['KMD']).replace('{','').replace('}', '')
                probe += '); %s}\n' % stack
                probes += probe
                if not driver:
                    driver = parts[2]
    ...
    def prepare_per_pid(self):
        ...
        if self.args.greedy:
            dtrace_script.append(GREEDY_UMD)
            # dtrace_script.append(GREEDY_KMD)  # FIXME: hangs the system so far
    ...
    def patch_per_pid(self, pids, items):
        ...
        if self.args.follow:
            fc = FOLLOW_CHILD.replace('/*{FOLLOW_CHILD}*/', self.fifo_reader.pipe)
            for pid in pids:
                result.append(fc.replace('$target', str(pid)))
        return result
